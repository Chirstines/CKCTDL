    ...
    public HashMap<String, StudyProgram> searchInList() {
        ...
        if (hasSelectedUniversity && !hasSelectedProgramName) {
            for (int i = 0; i < model.getProgramList().size(); i++) {
                if (model.getProgramList().get(i).getUniversity().getUniversityName()
                	.toLowerCase().contains(selectedUniversityName.toLowerCase())) {
                    existingProgramsMap.put(model.getProgramList().get(i).getProgramCode(), 
                    						model.getProgramList().get(i));
                }
            }
        }
        ...
        if (!hasSelectedUniversity && hasSelectedProgramName) {
            for (int i = 0; i < model.getProgramList().size(); i++) {
                if (model.getProgramList().get(i).getProgramName().toLowerCase()
                		.contains(selectedProgramName.toLowerCase())) {
                    existingProgramsMap.put(model.getProgramList().get(i).getProgramCode(), 
                    						model.getProgramList().get(i));
                }
            }
        ...
        if (hasSelectedUniversity && hasSelectedProgramName) {
            for (int i = 0; i < model.getProgramList().size(); i++) {
                if (model.getProgramList().get(i).getUniversity().getUniversityName()
                	.toLowerCase().contains(selectedUniversityName.toLowerCase()) &&
                    model.getProgramList().get(i).getProgramName().toLowerCase()
                    .contains(selectedProgramName.toLowerCase())) {
                    existingProgramsMap.put(model.getProgramList().get(i).getProgramCode()
                    						, model.getProgramList().get(i));
                }
